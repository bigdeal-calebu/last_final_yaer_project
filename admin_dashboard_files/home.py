...
def show_home_content(content_area, responsive_manager, admin_data=None, show_content_callback=None):
    ...
    ctk.CTkLabel(content_area, text="Dashboard Overview", font=("Segoe UI", 28, "bold"), text_color="#3b9dd8").pack(anchor="w", padx=30, pady=(30, 20))
    
    # Statistics Cards (4 cards)
    stats_frame = ctk.CTkFrame(content_area, fg_color="transparent")
    stats_frame.pack(fill="x", padx=30, pady=20)

    # getting total students - OPTIMIZED with error handling
    total_students = 0
    try:
        conn = get_connection()
        if conn:
    ...
    # Count unknowns (folders in unknowns/ directory)
    unknown_count = 0
    ...
    responsive_manager.register_grid(buttons_frame, 4)
