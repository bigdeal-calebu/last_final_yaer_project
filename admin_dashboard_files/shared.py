...
class CameraManager:
    ...
    def switch(self, index):
        if index == self.active_index:
            return True
        print(f"[Camera] Switching to {index}, releasing previous camera.")
        self.release_active()
    ...
    def release_active(self):
        """Release ONLY the currently active camera to turn off the hardware LED."""
        with self.lock:
            cap = self.pool.get(self.active_index)
            if cap:
                try: cap.release()
                except: pass
                del self.pool[self.active_index]
        print(f"[Camera] {self.active_index} released and turned OFF.")

# Global Singleton
manager = CameraManager()
# We NO LONGER call manager.warmup() at startup to keep the camera light off.
        ...
        if start_date_str:
            try:
                ...
                if datetime.now().date() < s_date:
                    # Future date set: Reset everything
...
def run_in_background(task_func, callback=None, args=(), kwargs={}):
    """
    Executes a function in a background thread.
    Use with caution: heavy UI updates should still be inside .after()
    """
    import threading
    def wrapper():
        try:
            ...
            if callback:
                # Callback should handle UI thread-safety if needed
                callback(result)
        except Exception as e:
