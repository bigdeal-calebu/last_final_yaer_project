...
def show_update_users_content(content_area, responsive_manager):
    ...
    search_frame.pack(fill="x", padx=30, pady=(0, 10), ipadx=20, ipady=15)
    
    search_row = ctk.CTkFrame(search_frame, fg_color="transparent")
    search_row.pack(fill="x", pady=10)
    
    ctk.CTkLabel(search_row, text="Search (Name / Reg No):", font=("Arial", 12, "bold"), text_color="white").pack(side="left", padx=10)
    search_var = ctk.StringVar()
    search_entry = ctk.CTkEntry(search_row, textvariable=search_var, width=300, placeholder_text="e.g. John or REG-001...")
    ...
    def perform_search():
        ...
        for student in results:
            ...
            ctk.CTkLabel(row_frame, text=info_text, text_color="white", font=("Arial", 13, "bold")).pack(side="left", padx=15, pady=10)
            
            edit_btn = ctk.CTkButton(row_frame, text="✏️ EDIT", width=80, fg_color="#3498DB", hover_color="#2980B9", font=("Arial", 12, "bold"), text_color="white")
            edit_btn.configure(command=lambda s=student: load_student_into_form(s))
            edit_btn.pack(side="right", padx=15, pady=10)
            
        results_container.pack(fill="x", pady=10)

    ctk.CTkButton(search_row, text="SEARCH", width=120, fg_color="#F39C12", hover_color="#D68910", text_color="black", font=("Arial", 12, "bold"), command=perform_search).pack(side="left", padx=10)
    ...
    grid_container.pack(fill="x", padx=20, pady=10)
    ...
    s_wrapper.grid(row=0, column=1, sticky="ew", padx=(5, 0))
