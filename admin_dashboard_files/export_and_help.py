...
def show_export_content(content_area, responsive_manager):
    ...
    ctk.CTkLabel(header_frame, text="📥 Export Data", 
                 font=("Segoe UI", 24 if is_small else 32, "bold"), text_color="#3498DB",
                 wraplength=wrap_len).pack(anchor="w")

    export_frame = ctk.CTkFrame(content_area, fg_color="#1a1a1a", corner_radius=15)
    export_frame.pack(fill="x", expand=True, padx=side_pad, pady=(10, 60 if is_small else 20))
...
def show_help_content(content_area, responsive_manager):
    ...
    ctk.CTkLabel(header_frame, text="❓ HELP & DOCUMENTATION", 
                 font=("Segoe UI", 24 if is_small else 32, "bold"), text_color="#3b9dd8",
                 wraplength=wrap_len).pack(anchor="w")
    
    ctk.CTkLabel(header_frame, text="Find answers, guides, and support for the system.", 
                 font=("Arial", 11 if is_small else 12), text_color="gray",
                 wraplength=wrap_len).pack(anchor="w", pady=(5, 0))

    
    # Search Bar
    search_frame = ctk.CTkFrame(content_area, fg_color="#1a1a1a", corner_radius=12)
    search_frame.pack(fill="x", padx=side_pad, pady=(10, 20))
    
    search_inner = ctk.CTkFrame(search_frame, fg_color="transparent")
    search_inner.pack(fill="x", padx=15 if is_small else 20, pady=15)
    
    search_entry = ctk.CTkEntry(search_inner, placeholder_text="🔍 Search documentation...", 
                                height=40, font=("Arial", 13), fg_color="#2c2c2c", 
                                border_color="#3b9dd8", width=500 if not is_small else 240)
    search_entry.pack(side="top" if is_small else "left", fill="x" if is_small else "none", padx=(0, 10 if not is_small else 0), pady=(0, 10 if is_small else 0))
    
    ctk.CTkButton(search_inner, text="SEARCH", fg_color="#3b9dd8", text_color="white",
                  width=120 if not is_small else 240, height=40, font=("Arial", 12, "bold")).pack(side="top" if is_small else "left", fill="x" if is_small else "none")
    
    help_scroll = ctk.CTkFrame(content_area, fg_color="transparent")
    help_scroll.pack(fill="both", expand=True, padx=side_pad, pady=(0, 20))
    ...
    ctk.CTkLabel(quick_start_header, text="🚀 QUICK START GUIDES", 
                 font=("Arial", 12 if is_small else 16, "bold"), text_color="white", wraplength=wrap_len).pack(pady=10)
    ...
    for title, description, color in quick_guides:
        ...
        info_frame.pack(side="top" if is_small else "left", fill="x", expand=True)
        ctk.CTkLabel(info_frame, text=title, text_color=color, font=("Arial", 14, "bold"), wraplength=wrap_len).pack(anchor="w")
        ctk.CTkLabel(info_frame, text=description, text_color="gray", font=("Arial", 11), wraplength=wrap_len).pack(anchor="w", pady=(3, 0))
        ctk.CTkButton(content_frame, text="View Guide →", fg_color=color, width=130 if not is_small else 200, height=35, font=("Arial", 11, "bold")).pack(side="top" if is_small else "right", pady=(10 if is_small else 0, 0))
    ...
    ctk.CTkLabel(features_header, text="📚 FEATURE DOCUMENTATION", font=("Arial", 12 if is_small else 16, "bold"), 
    ...
    for title, description in features:
        ...
        info.pack(side="top" if is_small else "left", fill="x", expand=True)
        ctk.CTkLabel(info, text=title, text_color="darkorange", font=("Arial", 13, "bold"), wraplength=wrap_len).pack(anchor="w")
        ctk.CTkLabel(info, text=description, text_color="gray", font=("Arial", 10), wraplength=wrap_len).pack(anchor="w", pady=(2, 0))
        ctk.CTkButton(row_content, text="Read More", fg_color="#3498DB", width=110 if not is_small else 200, height=30, font=("Arial", 10, "bold")).pack(side="top" if is_small else "right", pady=(10 if is_small else 0, 0))


    # FAQ
    faq_header = ctk.CTkFrame(help_scroll, fg_color="#F39C12", corner_radius=10, height=45)
    faq_header.pack(fill="x", pady=(25, 15))
    ctk.CTkLabel(faq_header, text="💬 FREQUENTLY ASKED QUESTIONS", font=("Arial", 12 if is_small else 16, "bold"), 
                 text_color="white", wraplength=wrap_len).pack(pady=10)
    ...
    for question, answer in faqs:
        ...
        # Add bottom padding specifically for the very last FAQ card
        is_last = (question == faqs[-1][0])
        faq_card.pack(fill="x", pady=6 if not is_last else (6, 60))
        
        faq_content = ctk.CTkFrame(faq_card, fg_color="transparent")
        faq_content.pack(fill="x", padx=20, pady=12)
        ctk.CTkLabel(faq_content, text=f"Q: {question}", text_color="#F39C12", font=("Arial", 12, "bold"), 
                     anchor="w", wraplength=wrap_len).pack(anchor="w")
        ctk.CTkLabel(faq_content, text=f"A: {answer}", text_color="lightgray", font=("Arial", 11), 
                     anchor="w", wraplength=wrap_len).pack(anchor="w", pady=(5, 0))
