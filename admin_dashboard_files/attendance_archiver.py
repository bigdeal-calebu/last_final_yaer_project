...
def sync_live_excel(date_str):
    ...
    try:
        ...
        _create_excel(
            ...
            [[r['name'], r['reg_no'], r['course'], r['program'], str(r['time_in'])] for r in present_records],
            "00C853" # Green
        )
...
def _ensure_db_record(filename, date_str, category, path):
    """Connects to the archive table and ensures this file is recorded exactly once."""
    from db import get_all_archives, record_archive_entry
    archives = get_all_archives()
    # If this filename isn't in our history yet, record it.
    if not any(a['filename'] == filename for a in archives):
