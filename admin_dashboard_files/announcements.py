...
def show_announcement_content(content_area, responsive_manager, admin_data=None):
    ...
    ctk.CTkLabel(header_frame, text="📢 Announcements", 
                 font=("Segoe UI", 24 if is_small else 34, "bold"), text_color="#F39C12",
    ...
    card_container.pack(fill="x", expand=True, padx=side_pad)
    ...
    inner_padding.pack(fill="both", expand=True, padx=15 if is_small else 40, pady=20 if is_small else 40)

    ctk.CTkLabel(inner_padding, text="Create New Announcement", font=("Segoe UI", 18 if is_small else 20, "bold"), 
    ...
    aud_side = "top" if is_small else "left"
    audience_opt = ctk.CTkOptionMenu(audience_frame, values=["All Users", "Students Only", "Staff Only", "Admins Only", "Specific Student"], width=250 if not is_small else 220, height=40, fg_color="#333", button_color="#444", button_hover_color="#555")
    audience_opt.pack(side=aud_side, pady=(0, 10) if is_small else 0)
    ...
    def on_audience_change(choice):
        if choice == "Specific Student":
            reg_no_entry.pack(side=aud_side, padx=(0 if is_small else 15, 0), pady=(0, 10) if is_small else 0)
        else:
    ...
    bottom_frame.pack(fill="x", pady=(0, 60 if is_small else 0))
    
    submit_btn = ctk.CTkButton(bottom_frame, text="📢 POST ANNOUNCEMENT", height=50 if is_small else 45, width=220 if not is_small else 250, 
                 font=("Segoe UI", 14, "bold"), fg_color="#F39C12", hover_color="#D68910", text_color="black", command=post_announcement)
    submit_btn.pack(side="top" if is_small else "right", fill="x" if is_small else "none")
